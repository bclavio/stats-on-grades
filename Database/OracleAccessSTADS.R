...
#listOfTables<-dbListTables(jdbcConnection) # large
#listOfTables<-as.data.frame(listOfTables) # not meningful
stamdata_hdk<-dbGetQuery(jdbcConnection,'select * from vueudv.aau_stamdata_hdk')

# From Bianca: I get errors from these two lines. Removed, as they seem redundant.
