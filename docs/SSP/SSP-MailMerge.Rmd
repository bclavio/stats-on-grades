...
studentHours['DotPos'] <- 35 # where the position the dot: sample size / 2

ggplot(studyHours, aes(x=rowID,y=hours)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) +
  #geom_jitter(height = 0, width = 0.1) +
  #geom_boxplot(width=0.1) + theme_minimal() +
  scale_y_continuous(breaks=seq(0,100,5), limits = c(0,75)) +
  coord_flip() +
...
#studyingHours <- 38
# Study and work
selectRecommondations[6,1] <- ifelse(studyingHours>=65, "much higher than the recommended",ifelse(studyingHours>=47, "sufficient",
...
#### Time Commitments (study and other)  - Note: focus here
...
You reported planning to use `r studyingHours` hours weekly for studying, which indicates that you are using
...
During your eduation, you should carefully review your commitments and other activities that you need or want to dedicate time to vis-&agrave;-vis the study demands. Should you not able to dedicate around 45 hours to study each week, you should carefully plan these hours and perhaps even minutes, so you do n0t overwork in wrong places. As students progress they get better at adjusting their study hours and become more aware of how much time they should dedicate to the different study activities. It is quite natural for students to underestimate the workload requirement in the first year. The adjustment is a process, and you just started your transistion to university life. This takes time, for some shorter for others longer, but we all have to go through changes like these in our lives.
...
### Student counseler services
If you need help for specific challenges as a student, we recommend you contacting a student counsler. Unlike counslers at high schools, student counslers at AAU are students studying themselves. They know many of the challenges that a Medialogy student faces, having experienced it themselves or helped fellow students. The student counsler is a good resource for handling student challenges such as:
