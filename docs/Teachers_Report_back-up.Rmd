...
names(work2Table)[1]<-"Non-related"
...
avgGrades <- NULL
avgGrades['Understanding of Medialogy'] <- list(rowMeans(dfSSPgradesStat[98:105]))
...
avgGrades['High School Study Habits'] <- list(rowMeans(dfSSPgradesStat[34:45]))
...
avgGrades['Social Support for Studying'] <- list(rowMeans(dfSSPgradesStat[2:8]))
...
ggplot(avgGradesMelt,aes(x = value)) + 
  facet_wrap(~variable,scales = "free_x") + 
  geom_density() + 
  scale_x_continuous(breaks=seq(0,0.1,0.05), limits = c(0,0.11)) +
  scale_y_continuous(breaks=seq(0,60,20), limits = c(0,70)) +
  labs(title = "Kernel density estimates", x = "Average of test scores")
