---
title: "Short analysis of study plan"
output: pdf_document
---
...
library(survminer)
library(ggplot2)
dfEnrolStatusBsc <- read.csv("Y:/dfEnrolStatusBsc.csv"); dfEnrolStatusBsc[,c(19,20)]<- NA
enrollMaxExamTry <- read.csv("Y:/enrollMaxExamTry.csv"); enrollMaxExamTry[,c(2,20)]<- NA
```
```{r,echo=FALSE}
enrolldata <- dfEnrolStatusBsc
enrolldata$fradatosn <- as.Date(as.character(enrolldata$fradatosn), "%Y-%m-%d")
enrolldata$slutdatosn <- as.Date(as.character(enrolldata$slutdatosn), "%Y-%m-%d")
...
```
Survival curves for students by year. Event of interest is dropout in general.

```{r, fig.height=5, fig.width=10,echo=FALSE}
sur <- survfit(Surv(Days,delta)~startaar, data = enrollYear)
ggsurvplot(sur, conf.int = T, risk.table = TRUE, break.time.by=100)
```
The curves show the pattern that students drop out earlier in 2014.

Survival curves for students where the event of interest is that they decide to drop out themselves.

```{r,fig.height=5, fig.width=10,echo=FALSE}
sur <- survfit(Surv(Days,deltaSelv)~startaar, data = enrollYear)
ggsurvplot(sur, conf.int = T, risk.table = TRUE, break.time.by=100)
```
This does not show as clear a tendency. 

Survival curves for students where the event of interest is that the institution does not allow them to continue.

```{r,fig.height=5, fig.width=10,echo=FALSE}
sur <- survfit(Surv(Days,deltaInst)~startaar, data = enrollYear)
ggsurvplot(sur, conf.int = T, risk.table = TRUE, break.time.by=100)
```
This shows much more clearly the same tendency, and it might indicate that the changes in 2014 are due to new rules and not the new study plan.
...
This observation is investigated further by looking at the ratio of students that have dropped out overall and separated by the different reasons in the next three plots.

```{r,echo=FALSE, fig.height=4}
navne <- c('2011'='red','2012'='green','2013'='purple','2014'='blue')
(ggplot()+ylim(0,1)+ylab('Dropout overall')+xlab('Semester') + geom_point(aes(factor(sem),drop2011, col='2011')) + geom_line(aes(sem,drop2011, col='2011'))
...
```
It indicated that the institution used to stop many students on fifth semester but in 2014 it is mostly on third semester.


Some further plots indicating the maximum times a student has attempted an exam, to see if there is some connection
```{r,echo=FALSE}
...
ggplot(dat, aes(factor(Var2), Freq, fill = Var1)) + 
  geom_bar(stat="identity", position = "dodge")+xlab('Max exam try') +
    ggtitle('Barplot for all students')
```
...
ggplot(datInst, aes(factor(Var2), Freq, fill = Var1)) + 
  geom_bar(stat="identity", position = "dodge") +xlab('Max exam try') +
    ggtitle('Barplot for students that were not allowed to continue')
...
ggplot(datSelv, aes(factor(Var2), Freq, fill = Var1)) + 
  geom_bar(stat="identity", position = "dodge") +xlab('Max exam try') +
    ggtitle('Barplot for students that decided to drop out')
...
ggplot(datFinished, aes(factor(Var2), Freq, fill = Var1)) + 
  geom_bar(stat="identity", position = "dodge")+xlab('Max exam try') +
    ggtitle('Barplot for students that finished')
